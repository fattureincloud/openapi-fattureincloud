...
public class RetryingOAuth extends OAuth implements Interceptor {
   ...
   *     OAuth2 flows: "password", "clientCredentials" and "authorizationCode". The value must be an
   *     absolute URL.
  ...
  private Response retryingIntercept(Chain chain, boolean updateTokenAndRetryOnAuthorizationFailure)
    ...
    if (getAccessToken() != null) {
      ...
      // 401/403 response codes most likely indicate an expired access token, unless it happens two
      // times in a row
      if (response != null
